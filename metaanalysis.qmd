...
load_packages <- function(packages) {
  # Clean package names (remove versions like 'tidyverse@1.3.0')
  packages_clean <- gsub("[^[:alnum:]_].*$", "", packages)
  
  # Check which packages are NOT installed
  installed_pkgs <- rownames(installed.packages())
  to_install <- packages[!packages_clean %in% installed_pkgs]
  
  # Install only missing packages using renv
...
```{r}
quadas_raw <- read_tsv("data/quadas_v2.tsv")

quadas <- quadas_raw %>% 
  rename(Study = Domain) %>% 
  pivot_longer(-Study, names_to = "Domain", values_to = "Score") %>%
  mutate(
...
```{r}
data_raw <- read_tsv("data/studies_stats_v2.tsv")
...
# Descriptive analysis: pooled sensitivity, specificity, PPV and NPV
